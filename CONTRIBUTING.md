...
- **The build command copies the deployed files.** Only files it names reach the deploy directory, so adding another deployed file means adding it to the build command, or the new file silently never ships. The host's build watch path is a wildcard, so any change to the repository triggers a rebuild. The build command is `mkdir -p dist/.well-known && cp index.html robots.txt sitemap.xml llms.txt dist/ && cp .well-known/ai-catalog.json dist/.well-known/`. The catalog is copied separately because `cp` drops folders, and Lighthouse only looks for it under `/.well-known/`.
