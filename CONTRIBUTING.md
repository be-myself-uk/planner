...
- `loadUrlParams()` / `handleUrlLoad()` / `handleLegacyUrl()` — the reverse, reconstructing state from a shared link. Read these functions directly in `index.html` if you need the exact data format; it isn't reproduced here.
