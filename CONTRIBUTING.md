...
- `REGION`: `{ EW, WALES, SCOT, NI }`. `EW` means England specifically (its value is still the string `'ew'` for backwards compatibility with existing shared links); `WALES` is the current-residence region only, not a fourth birth-registration region, since England and Wales share one civil registration system for births — `birthRegion` only ever takes `EW`/`SCOT`/`NI`/`out`, never `WALES`. Because `planItemRegion()` falls back to a `PLAN_ITEMS` entry's `default` variant for any region key it doesn't recognise, Wales needs its own `regions.wales` entry only where it genuinely diverges from England (currently just the gender-service waiting-list note in `buildHealthItems()`); everything else (DBS, DWP, deed poll, GRC, driving licence) falls straight through to `default` with no duplicated content. Note that "outside the UK" is not a fifth region value. It is tracked as a separate boolean flag (for example, `regionOutsideUK`, `birthOutsideUK`) alongside a region that falls back to `EW`, because most of the app's regional logic only needs to distinguish EW/Wales/Scotland/NI. `regionOutsideUK`/`birthOutsideUK` are checked directly wherever UK-residency-specific content needs to be skipped or disclaimed instead: `planParamsFromWizard()` and `updateLocks()` gate the NHS/GP, DBS, DWP, and Council Tax/electoral register content off when `regionOutsideUK` is set, the `nhs`/`newGP`/`dbs`/`dwp` wizard questions use it in their `cond`, and it can select an `out` variant on a `PLAN_ITEMS` entry (see `birthcert`) the same way `ni`/`scot` do.
