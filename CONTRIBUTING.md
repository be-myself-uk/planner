...
- `REGION`: `{ E, W, S, NI }` (values `'e'`, `'w'`, `'s'`, `'ni'`). `E` means England specifically. `REGION_LEGACY` maps the older values a shared link might still encode (`ew`, `wales`, `scot`, from before the region codes were shortened) to their current equivalents; `resolveRegion(v)` applies that mapping and is called wherever a region value arrives from outside the app (`loadUrlParams`, the `isValid` check in `handleUrlLoad`), so old shared links keep working. The same four values are used for both the current-residence region (`wizardState.region`) and the birth-registration region (`wizardState.birthRegion`), asked as two separate questions, since they legitimately differ (a person can live in one nation and have been born in another). England and Wales are split in both for consistency, even though `birthRegion` currently treats them identically (both share the General Register Office), because that could change independently in future and a distinct `W` value costs nothing today. Because `planItemRegion()` falls back to a `PLAN_ITEMS` entry's `default` variant for any region key it doesn't recognise, Wales needs its own `regions.wales` entry only where it genuinely diverges from England: today that's just the gender-service waiting-list note in `buildHealthItems()` (keyed off `wizardState.region`); nothing currently keys any content off `birthRegion === REGION.W` specifically, so a Wales-born person gets identical birth-certificate/GRC content to an England-born one, the same way NI-born and Scotland-born get their own content by comparing against `REGION.NI`/`REGION.S` and everything else (including both `E` and `W`) falls through unchanged. Note that these `PLAN_ITEMS.regions` keys (`ni`, `scot`, `wales`, `default`) are an independent, human-readable naming convention for content lookup, not tied to `REGION`'s own short values. Note also that "outside the UK" is not a fifth region value. It is tracked as a separate boolean flag (for example, `regionOutsideUK`, `birthOutsideUK`) alongside a region that falls back to `E`, because most of the app's regional logic only needs to distinguish England/Wales/Scotland/NI. `regionOutsideUK`/`birthOutsideUK` are checked directly wherever UK-residency-specific content needs to be skipped or disclaimed instead: `planParamsFromWizard()` and `updateLocks()` gate the NHS/GP, DBS, DWP, and Council Tax/electoral register content off when `regionOutsideUK` is set, the `nhs`/`newGP`/`dbs`/`dwp` wizard questions use it in their `cond`, and it can select an `out` variant on a `PLAN_ITEMS` entry (see `birthcert`) the same way `ni`/`scot` do.
...
- **`evidence` / `evidenceFn(p)`**: an optional field on any `PLAN_ITEMS` leaf, rendering a tick-off checklist of the supporting documents that step needs. Shaped `{ idPrefix, intro?, items: [{ k, label }] }`, resolved by `planItemEvidence(leaf, p)`, which prefers `evidenceFn(p)` when the list depends on the user's answers (see `grc_docs`) and otherwise uses the fixed `evidence` object (see `grc_med`, `grc_life`). Because it sits on a leaf it varies by region and variant exactly as `summary` and `detail` do. Each entry renders through the shared `subItem()` helper as an ordinary tracked sub-item, so parent/child status syncing, "Reset progress", and shared links all work with no extra handling.
  ...
  Each entry's `k` becomes part of its stored id (`st_trk_<idPrefix>_<k>`) and of the shared-link payload, so **never renumber or reuse a key**: deriving ids from array position instead would silently reassign someone's saved ticks the moment a list is reordered or an entry inserted. Keep evidence single-level — the parent/child sync logic is one deep, and grandchildren would break it. Every label is user-facing guidance, so a new one is a factual claim that needs a `SOURCES.md` entry like any other, and `content-snapshot-lib.js` captures both `evidence` and `evidenceFn` so wording changes show up in the content-integrity test.
