...
├── llms.txt                       : short Markdown summary of the planner for AI tools (llmstxt.org format); copied to the deploy directory by the build command
├── .well-known/ai-catalog.json    : Agentic Resource Discovery catalog, deliberately with no entries; copied to the deploy directory by the build command
...
- **The build command copies the deployed files.** Only files it names reach the deploy directory, and only paths in the host's build watch list trigger a rebuild. Both currently cover `index.html`, `robots.txt`, `sitemap.xml`, `llms.txt`, and `.well-known/ai-catalog.json`; adding another deployed file means updating both, or the new file silently never ships. The build command is `mkdir -p dist/.well-known && cp index.html robots.txt sitemap.xml llms.txt dist/ && cp .well-known/ai-catalog.json dist/.well-known/`. The catalog is copied separately because `cp` drops folders, and Lighthouse only looks for it under `/.well-known/`.
- **Unmatched paths serve `index.html` with HTTP 200**, because there is no top-level `404.html`. `robots.txt`, `sitemap.xml`, `llms.txt`, and `.well-known/ai-catalog.json` are real files specifically so that crawlers and AI tools requesting them get valid content rather than the application HTML.
- **`llms.txt` and `ai-catalog.json` exist for Lighthouse's Agentic Browsing checks.** `llms.txt` must be at least 50 characters, contain a `# ` heading and at least one Markdown link, and should say the same things as the README. `ai-catalog.json` must be valid JSON with `"specVersion": "1.0"` and an `entries` array, and Lighthouse rejects any root or `host` field outside its schema. Its `entries` are empty on purpose: entries describe AI agents, tool servers, or APIs, and the planner has none. Without the catalog, the host's `index.html` fallback answers `/.well-known/ai-catalog.json` with HTML, which Lighthouse reports as a broken catalog rather than a missing one. Tests 179 and 180 check the two files against these rules.
